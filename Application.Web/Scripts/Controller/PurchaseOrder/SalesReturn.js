function populateSalesReturnPurchaseOrderDetails(callback) {
    ...
    $.ajax({
        ...
        success: function (order) {
            ...
            window.order = order;
            renderSalesReturnPurchaseOrderItems(window.order);
        },
        error: function (xhr) {
            $('.item-loading').hide();
        }
...
function renderSalesReturnPurchaseOrderItems(order) {
    ...
    html += '<td class="center" style="width:150px;">L Recv Q</td>';
    ...
    for (var i = 0; i < order.OrderItems.length; i++) {
        ...
        let recvQuantityInputBoxId = 'txtRecvQty_' + item.Id;
        ...
        html += '<input type="number" min="0" class="dynamic-inputbox data-change" style="width:50px; text-align:center;" value="' + item.LastReceivedQuantity+'" itemid="' + item.Id + '" id="' + recvQuantityInputBoxId + '" />';
        html += '</td>';
    ...
    $('#purchase-order-details-item-list').html(html);
}

$('#purchase-order-details-item-list').on('change', '.data-change', function () {

    var itemId = $(this).attr('itemid');
    var qty = $('#txtQty_' + itemId).val();
    let recvQty = $('#txtRecvQty_' + itemId).val();
    ...
    for (var i = 0; i < window.order.OrderItems.length; i++) {
        ...
        if (item.Id === itemId) {
            ...
            window.order.OrderItems[i].LastReceivedQuantity = parseInt(recvQty, 10);
...
$('#purchase-order-details-item-list').on('change', '.discount-change', function () {
...
$('#btnUpdatePurchaseOrder').click(function () {
    ...
    for (var i = 0; i < orderItems.length; i++) {
        ...
        let recvQuantity = parseInt(orderItems[i].LastReceivedQuantity, 10);

        orderItem.ProductId = orderItems[i].ProductId;
        orderItem.Quantity = quantity;
        orderItem.ReceivedQuantity = recvQuantity;
...
app.controller('PurchaseSalesReturnCtrl', ['$rootScope', '$scope', '$http', '$filter', '$location', 'Enum', 'service', function ($rootScope, $scope, $http, $filter, $location, Enum, service) {

    var orderId = getParam('orderId');

    if (orderId) {
        populateSalesReturnPurchaseOrderDetails(orderId);
    }
    ...
    $('#purchase-order-details-item-list').on('click', '.delete-shopping-cart-item', function () {
        var id = $(this).attr('id');
        removeItem(id)
